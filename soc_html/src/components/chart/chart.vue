<template>
    <div class="pos-r">
        <div v-show="status==0"
             class="textC"
             v-bind:style="{width: '100%', height: box.height+'px',lineHeight: box.height+'px'}">
            <loading-normal :show="true"></loading-normal>
        </div>
        <div v-show="status==1"
             class="textC ys-info-color"
             v-bind:style="{height: box.height+'px',lineHeight: box.height+'px'}">
        ...
        <div v-show="status==2">
            ...
            <div class="" style="height:100%;width:100%">
                ...
                <!-- 表格类型 -->
                <div class="ys-box" :style="{height:box.height+'px',paddingTop:'10px',margin:'auto'}"
                     v-if="type=='table'">
                    <div id="attack-event-24-hour-box">
                        <table class="ys-chart-table">
                            <thead>
                            <tr>
                                <th v-for="item in listThead">{{item}}</th>
                            </tr>
                            </thead>
                            <tbody>
                            <tr v-for="(i,item) in listTbody" track-by="$index">
                                <td v-for="(j,hitem) in item" track-by="$index">{{listTbody[i][j]}}</td>
                            </tr>
                            </tbody>
                        </table>
                ...
                <!-- 散点图 -->
                <scatter-chart v-if="type=='scatter'&&newDataUp==true"
                               :height="box.height"
                               :unit="unit"
                               :datas="datas"></scatter-chart>
                <!-- 环形图 -->
                <loop-chart v-if="type=='loop'&&newDataUp==true"
                            :height="box.height"
                            :datas="datas"
...
<script>
    ...
    import lingBarChart from './ling-bar-chart'
    ...
    export default {
        ...
        data() {
            return {
                ...
                listThead: [],
                listTbody: [],
                attackEvent24HourData: [ //table 假数据
                    ...
                    tooltip: {
                        ...
                        formatter: function (params) {
                            ...
                            for (var i = 0; i < params.length; i++) {
                                ...
                                } else if (self.unit == "time") {
                                    str += "<br/>" + params[i].seriesName + ":" + self.changeTime(params[i].value)
                                } else {
                ...
                let option = {
                    tooltip: {
                        ...
                        formatter: function (params) {
                            ...
                            if (self.unit == "band") {
                                value =  self.changeUnit(value)
                            } else if (self.unit == "number") {
                                value =  self.changeNumber(value)
                            } else if (self.unit == "time") {
                                value =  self.changeTime(value)
                            }
                            return params.data.name + "<br/>总量：" + value + "<br/>占比：" + params.percent + "%"
                        }
                    },
                    color: [
